...
| V111-043 | P1 | Offline boot | An installed v111 PWA forces a worker update while offline and refuses to load its complete cached runtime | Release Engineering | Open | Cold controlled-v111 offline reload must load every cached runtime asset |
| V111-044 | P2 | Boot recovery | Failure to fetch the sole v111 boot script leaves the HTML shell blank with no retry path | Release Engineering | Open | Dependency-free HTML recovery surface remains visible and reloadable when boot never executes |
| V111-045 | P2 | Boot failure isolation | Adding the boot-failed class exposes all raw application screens before the main stylesheet loads | Release Engineering | Open | Every failure path shows only the dependency-free recovery surface |
| V111-046 | P2 | Architecture gate | Loader-order test still inspects removed direct index scripts instead of the approved boot graph | Release Engineering | Open | Complete architecture suite validates and passes against the atomic loader |
| V111-047 | P2 | Settings migration | A complete canonical object written by v110 mute handling can contain stale defaults that overwrite visibly saved setup preferences | UI/State | Open | Divergent complete-v110 fixture preserves feel, feedback, and reduced motion exactly once |
| V111-048 | P1 | Atomic runtime | A dynamically loaded script can throw during execution yet still fire `load`, allowing boot-ready on a partial runtime | Release Engineering | Open | Injected middle-script exception aborts all later loads and never reaches boot-ready |
