...
| V111-054 | P1 | Stats import lineage | A later export from the same source drops a growing retention rollup, reducing a 100-flip source to 3 imported flips | Stats/Data | Open | Import 2-flip then 100-flip snapshots into one destination; summary must remain exactly 100 and repeat import must be a no-op |
| V111-055 | P2 | Save-import names | Setup `rows[]` is not recognized as player context, so blocked names survive checksummed save import and persistence | State/Data/Safety | Open | Blocked setup row is blank after serialize/parse and before local-storage apply |
...
| V111-059 | P2 | Ice deadline verdict | The hard deadline awards a tilt-only make while the bottle is suspended about 95px above the scoring plane | Physics/Events | Open | Seed 3668341011 resolves by the deadline but off-plane is MISS, never `upright-settle-limit` |
