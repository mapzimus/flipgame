...
Download `flipgame-offline.apk` from the repository's `v111` release or the
...
cannot install v111 over that particular APK. If v110 is installed, uninstall
it once and then install v111. Export any local save or statistics you want to
keep first. v111 and future official APKs use the same protected release key,
...
available offline. A v111 service-worker update installs atomically, so a
...
immutable `v111` release, and refreshes `apk-latest`. GitHub Pages serves the
