...
> build forever. (Currently at **v110**.)
...
# PART 11 — SESSION UPDATE (current state: v110)
...
- **ON FIRE:** +1 life per flip, bounded by a fixed match ceiling of 150% of the selected starting-lives preset (odd totals round up). The old >4-player +5 run cap is removed; every lobby uses the same match ceiling. Reaching it passes the turn gracefully with no penalty.
- **SUDDEN DEATH** after `SD_THRESHOLD=70` flips (`game.js`): ordinary misses take an escalating extra penalty (`SD_STEP=20` flips/level). ON FIRE remains protected: makes add lives up to the match ceiling and the ending miss is free.
- **Seeded rare-event ladder** (`physics.js`, mutually exclusive per flick): Rainbow Trail 1/50 (a real six-band trail plus corkscrew flight), Power Launch 1/100, Moon Gravity 1/200, Alien Invasion 1/250, Ice Slide 1/300, Gravity Slam 1/400, Trampoline Table 1/500, Wind Tunnel 1/600, Double Flip 1/700, Magnet Landing 1/800, Heart Rush 1/900, and Life Drain 1/2000. Every event now has a strong arena treatment and more extreme physical motion, with event-specific landing assistance so none becomes a disguised automatic miss. Double Flip requires two full rotations; a successful upright or cap landing doubles the flipper and halves every active opponent. Life Drain uses a hidden strong magnet, sets every active opponent to one life on its successful landing, and leaves a green tint for the rest of the match.
- **Alien scoring:** Alien characters use a full-size 1:1 desktop arena and zero-G-style bank shots. A wall, ceiling, wedge, or saucer carom arms a floating tractor ring; passing through the armed ring scores the make. The 1/250 Alien Invasion event temporarily gives any object the same rule and is included in Insane Mode.
- **PLINKO DROP** remains 1/1000 and the only automatic-win mechanic. Its extended eight-row board has nine symmetric bins: `×2 lives | ½ others | Always Magnet | AUTO LOSS | AUTO WIN | AUTO LOSS | Always Magnet | ½ others | ×2 lives`. Always Magnet follows that player for the rest of the match; Auto Loss eliminates the flipper.
- **Plinko follow-cam:** during the long drop the camera stays close and tracks the falling object near screen center, then carries the reward bins into view at the bottom. This prevents the 8-player HUD from hiding the action.
- **Insane Mode:** unlocks only when Alien is owned. The setup disables it and safely falls back to Normal for stale saved settings until then. Every flip has a deterministic 1-in-3 special-event chance. Life Drain is excluded; the eleven remaining non-Plinko events (including Alien Invasion) are equally weighted and Plinko has 5 selection buckets versus 4 for each other event (25% higher individual weight). The in-game badge is simply `INSANE MODE`.
- **Special-event testing:** exact case-sensitive player name `Mr. Howe` multiplies every rare roll by 10 (for example 1/50 → 1/5 and Plinko 1/1000 → 1/100); it no longer guarantees an event every flip. Offline player names matching an event force it on every flip, case-insensitively and with or without spaces (`Rainbow Trail`, `Power Launch`, `Moon Gravity`, `Alien Invasion`, `Ice Slide`, `Gravity Slam`, `Trampoline Table`, `Wind Tunnel`, `Double Flip`, `Magnet Landing`, `Heart Rush`, `Life Drain`, or `Plinko Drop`). Short forms `Alien`, `Trampoline`, `Magnet`, and `Plinko` also work. `Trampoline Table` exceeds the 14-character roster limit, so its stored `Trampoline Tab` form is explicitly recognized too. The same names typed without spaces during gameplay arm only the next flip.
- **Visible version badge:** the lower-right corner always shows the running release (`v110`) on setup, gameplay, overlays, and game-over screens so stale installs are immediately obvious.
...
`SD_THRESHOLD`/`SD_STEP` (ordinary-turn sudden-death penalty), `MAX_LIFE_MULTIPLIER` (fixed additive-reward ceiling), `pointCount` semantics (start 0). Flick feel in `js/physics.js`: `POWER_SPEED=4000`, settle detection requires angVel<0.010, linSpeed<7, a 22-frame/0.030-rad stable window, and at least 30 grounded frames; the fallback waits 600 grounded frames and honors an upright final pose. Turn clock `TURN_SECONDS=10`/`FIRE_SECONDS=4`.
