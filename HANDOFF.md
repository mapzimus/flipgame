...
> build forever. (Currently at **v49**.)
...
- **Stale-cache gotcha:** the dev server sends no cache headers, so the browser serves stale JS on reload. Scripts/CSS carry a `?v=N` query (currently **v49**) — **bump it on every change** (and the matching `CACHE_NAME` in `service-worker.js`) or the browser/SW serves the old build. This is the #1 "my change didn't show up" cause.
