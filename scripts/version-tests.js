...
assert.equal(publicVersion, 'v1.11');
...
assert.ok(index.includes('version 1.11'), 'version badge accessibility label is stale');
...
for (const [file, html] of [['index.html', index]]) {
  ...
  assert.deepEqual([...new Set(versions)], ['111'], `${file} has mixed asset build identifiers`);
}

console.log(`Version consistency tests passed (${publicVersion}, build 111).`);
